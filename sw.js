...
self.addEventListener('push', function(event){
  ...
  const options = {
    ...
    renotify: data.renotify !== undefined ? data.renotify : !isPromo,
    requireInteraction: data.requireInteraction !== undefined ? data.requireInteraction : !isPromo,
    data: { url: data.url || (isPromo ? './' : './?dboy=1') }
  };
...
self.addEventListener('notificationclick', function(event){
  ...
  // already open" — e.g. 'dboy=1' for order alerts, or null (any tab) for
  // a general promo linking to the homepage.
  const matchFragment = targetUrl.indexOf('dboy=1') !== -1 ? 'dboy=1' : null;
